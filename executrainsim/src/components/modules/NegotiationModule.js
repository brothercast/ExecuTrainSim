...
const BACKEND_API_SERVER_URL = 'http://localhost:8080'; 
...
const NegotiationModule = ({ onReturn }) => {  
    ...
    const sendUserReply = async () => {  
        ...
        if (scores) {  
            ...
            let scaledProgress = (performanceScore + scoreSum) / scenario.goal * 100;  
    ...
    // Handle opponent's response from generateOpponentResponse (Ensures only one message is added)  
    useEffect(() => {  
        // This effect can be used if opponent responses are handled via state updates  
        // Currently, opponent responses are handled directly in generateOpponentResponse  
    }, [chatHistory]);  
