...
export const NegotiationModule = ({ onReturn }) => {
    // State variables
    ...
    // Refs for debouncing and preventing duplicate useEffect calls
    const isSendingReply = useRef(false);
     const hasGeneratedOptionsRef = useRef(false);
    const isOpponentRespondingRef = useRef(false);
  ...
  const generateScenario = async () => {
    ...
    // Generate user response based on selected strategy
     const generateUserResponse = async (strategyDescription) => {
        logMessage("generateUserResponse called with:", { strategyDescription });
    ...
    const createResponseOptionsPrompt = (context, latestOpponentMessage, previousUserMessage) => `
       ...
       consider the latest opponent message: "${latestOpponentMessage}"
        ...
        Generate four options as the basis for a response, each representing a different high-level negotiation tactic or strategy that the user could employ.
       Each option should describe a general approach the user could take, not specific messages or phrases.
          The response options should be general negotiation strategies or tactics such as (but definitely not limited to) the following:
           - Counter Offer
           - Ask for More
           - Play Hardball
           - Concede
   ...
   // Updated generateOpponentResponse function
     const generateOpponentResponse = async (lastUserMessage) => {
         if (isOpponentRespondingRef.current) {
              console.log("[DEBUG] generateOpponentResponse: Already responding, ignoring this call.");
            return; // Prevent multiple calls
         }
         isOpponentRespondingRef.current = true; // Set flag to indicate opponent responding
        logMessage('generateOpponentResponse: Starting', {lastUserMessage});
         try {
            const userRole = scenario.roles.find((r) => r.name === selectedRole);
            const opponentRole = scenario.roles.find((r) => r.name !== selectedRole);
            if (!userRole || !opponentRole) {
           ...
           const outcome = await assessNegotiationOutcome();
            if (outcome && outcome.outcome !== 'Draw') {
              finalizeSimulation();
              isOpponentRespondingRef.current = false; // Reset flag when done
                return;
            }
            isOpponentRespondingRef.current = false; // Reset flag when done
           ...
           console.error('Failed to generate opponent response:', error);
            setErrorMessage('Failed to generate opponent response. Please try again.');
             isOpponentRespondingRef.current = false; // Reset flag on error
           return null;
        }
    };
     // Send user reply and process AI response
   const sendUserReply = async () => {
       if (isSendingReply.current) {
            console.log("[DEBUG] sendUserReply: Already sending a reply, ignoring this call.");
           return; // Prevent multiple calls
       }
        isSendingReply.current = true; // Set flag to indicate sending
         logMessage(`sendUserReply: Function called at ${new Date().toLocaleTimeString()}`); // Logging at function start

        if (!userDraft.trim()) {
           setErrorMessage('Please type a reply before sending.');
           isSendingReply.current = false; // Reset flag on error
            return;
      }
        setErrorMessage('');
       ...
       // Send Response
         setTimeout(async () => {
          ...
          updateProgress(scaledProgress, scores);
           ...
           isSendingReply.current = false; // Reset flag after opponent response
        }, delay);
   };
   ...
   // Assess the final outcome of the negotiation
    ...
    const resetNegotiation = () => {
       ...
       setCustomSetupStage('selection');
         setPerformanceData([]);
       hasGeneratedOptionsRef.current = false; // Reset options generated flag on reset
   };
    ...
    useEffect(() => {
         if (scenario && negotiationStarted && isUserTurn) {
            if (!hasGeneratedOptionsRef.current) {
                 generateResponseOptions(scenario.context);
                hasGeneratedOptionsRef.current = true; // Mark options as generated for this turn
            } else {
               console.log("[DEBUG] useEffect: Response options already generated for this turn, skipping.");
            }
        } else {
            hasGeneratedOptionsRef.current = false; // Reset flag when it's not the user's turn or negotiation hasn't started
       }
   }, [scenario, negotiationStarted, isUserTurn]);

    // Metadata for the component
   NegotiationModule.metadata = {
                                ...
                                <>
                                    {!scenario && (
                                        <div className="module-description">
                                           ...
                                           {showInstructions && (
                                               <div>
                                                  {NegotiationModule.metadata.instructions.split('\n').map((line, i) => (
                                                      <p key={i}>{line}</p>
                                                  ))}
                                               </div>
                                           )}
